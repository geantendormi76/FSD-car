...
pub trait 车辆运动控制器: Send {
    ...
    fn 设置参考轨迹点(&mut self, stage: i32, ref_point: &Self::轨迹)
        -> Result<(), String>;
    fn 设置动态障碍物硬约束(&mut self, obstacles: &[Self::障碍]) -> Result<(), String>;
    fn 求解最优控制量(
        &mut self, 当前线速度: f64, dt: f64
    ) -> Result<Self::指令, String>;
}
...
impl 车辆运动控制器 for solver::预测控制求解器 {
    ...
    fn 设置参考轨迹点(
        &mut self, stage: i32, ref_point: &Self::轨迹
    ) -> Result<(), String> {
