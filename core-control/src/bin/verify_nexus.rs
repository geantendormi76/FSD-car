...
use core_control::{车辆运动控制器, 预测控制求解器, decode_platform_agnostic_actions};
...
fn main() -> Result<(), String> {
    ...
    println!("    -> Forward Limit Command: v_cmd={:.4} m/s, w_cmd={:.4} rad/s", v_f, w_f);
    assert!((v_f - 0.80).abs() < 1e-5, "[-] ERROR: Forward speed limit mapping failed!");
    assert!((w_f - 1.0).abs() < 1e-5, "[-] ERROR: Forward yaw rate limit mapping failed!");
    
    let (v_r, _, _) = decode_platform_agnostic_actions(-1.0, 0.0);
    println!("    -> Reverse Limit Command: v_cmd={:.4} m/s", v_r);
    assert!((v_r - (-0.30)).abs() < 1e-5, "[-] ERROR: Reverse speed limit mapping failed!");
    
    let (v_s, _, w_s) = decode_platform_agnostic_actions(0.0, -1.0);
    println!("    -> Pivot Turn Command   : v_cmd={:.4} m/s, w_cmd={:.4} rad/s", v_s, w_s);
    assert!(v_s.abs() < 1e-5, "[-] ERROR: Stationary velocity check failed!");
    assert!((w_s - (-1.0)).abs() < 1e-5, "[-] ERROR: Pivot turn singularity bypass failed!");
    ...
    let (v_cmd, w_cmd) = brain.求解最优控制量(current_velocity)?;
    println!("    -> Tracking Command: v_cmd={:.6} m/s, w_cmd={:.6} rad/s", v_cmd, w_cmd);
    assert!(v_cmd > 0.0, "[-] ERROR: Vehicle failed to accelerate forward!");
    assert!(w_cmd.abs() < 1e-4, "[-] ERROR: Clear road tracking generated unnecessary steering!");
    ...
    let (v_avoid, w_avoid) = brain.求解最优控制量(current_velocity)?;
    println!("    -> Avoidance Command: v_cmd={:.6} m/s, w_cmd={:.6} rad/s", v_avoid, w_avoid);
    
    // 🛡️ 物理断言：面对偏右障碍，MPC 应当安全地向左（w_avoid > 0）打舵绕行
    assert!(w_avoid.abs() > 0.01, "[-] ERROR: NMPC failed to dodge the off-center obstacle!");
    assert!(w_avoid > 0.0, "[-] ERROR: NMPC steered in the wrong direction!");
