...
fn local_to_bev(x: f64, y: f64) -> (i32, i32) {
    let res = 0.03125;
    let col = 96 + (x / res) as i32;
    let row = 191 - (y / res) as i32;
    (row, col)
}
...
async fn main() -> eyre::Result<()> {
    ...
    let _ = writeln!(log_file, "tick,cur_x,cur_y,cur_yaw,target_x,target_y,ttc,v_cmd,w_cmd,final_v");
        ...
        match event {
            Event::Input { id, data, .. } => {
                ...
                if id_str == "ttc" {
                    ...
                    let corridor = 生成BEV自适应凸包走廊(0.0, 0.0, local_target_x, local_target_y, &latest_bev_grid);
                    ...
                    let _ = writeln!(log_file, "{}, {:.4}, {:.4}, {:.4}, {:.4}, {:.4}, {:.4}, {:.4}, {:.4}, {:.4}",
                        tick_count, current_x, current_y, current_yaw, target_x, target_y, current_ttc, v_cmd, w_cmd, final_v_cmd);
                        
                    if tick_count % 100 == 0 {
                        let brake_status = if brake_factor < 0.5 { "RED EMERGENCY BRAKE" } else { "GREEN" };
                        print!("[NMPC Core] Tick: {:<5} | TTC: {:>4.1}s [{}] | Raw_V: {:.2} | Final_V: {:.2} m/s\r",
