...
async fn main() -> eyre::Result<()> {
    ...
    let 规控句柄 = tokio::spawn(async move {
        ...
        // 🎯 战役三严谨度自愈：虚拟障碍物位置低通滤波器状态
        // 彻底根治因感知噪点左右瞬移（sign反转）导致的 NMPC 求解器发散 (Status 4)
        ...
        loop {
            ...
            // 当小车接近静止（如刚起跑、终点驻车静默、任务结束）时，屏蔽任何由于相机晃动引发的虚警势场力，彻底抹平原地鬼畜
            ...
            // 🎯 SOTA 药方 2：自激振荡低通滤波阻尼 (PT1 Filter)
            let mut 侧向滤波系数 = 0.086f64;
            // 🚨 战役三 Rust 降噪屏障 B：极性交变阻尼 (Polar Flip Damper)
            // 如果感知输出的排斥力方向瞬间反转（例如从右侧瞬移到左侧），我们强行拉长其越过中轴线的物理过渡时间（降速 6 倍）
            // 阻止 NMPC 的硬椭圆墙在车身左右两侧瞬间横跳
            if 有效期望_y.signum() != 滤波后的期望_y.signum() && 滤波后的期望_y.abs() > 0.01 {
                侧向滤波系数 = 0.015f64; // 大幅增加极性交叉阻尼，抹平突变
            }

            滤波后的期望_x += 0.086f64 * (有效期望_x - 滤波后的期望_x);
            ...
            // 🎯 战役三核心：势场坍缩与虚拟障碍物生成 (Virtual Obstacle Collapse)
            // 当青蛙眼感受到强烈的侧向逃逸力或纵向排斥力时，在小车局部坐标系正前方“坍缩”出一个真实的物理椭圆禁区！
            let (目标障碍物_x, 目标障碍物_y, 椭圆长轴_a, 椭圆短轴_b) = if 抑制后的期望_y.abs() > 0.04 || 滤波后的期望_x < -0.04 {
                // 1. 纵向锚定：障碍物在小车正前方 0.6 米处（稍微拉远前视距离，留给 NMPC 更多的减速缓冲裕度）
                let obs_x = 0.6;
                // 2. 横向锚定：使用平滑线性映射代替 discontinuous 的 signum() 函数！
                // 抑制后的期望_y 在 [-1.5, 1.5] 之间。我们将其平滑限幅在 [-0.35, 0.35] 范围内。
                // 既保留了方向极性，又杜绝了零点附近的阶跃跳变！
                let oy = -抑制后的期望_y.clamp(-0.35, 0.35); 
                // 3. 设定膨胀椭圆的安全半轴 (长轴 35cm，短轴 25cm)
                (obs_x, oy, 0.35, 0.25)
            } else {
                // 安全期：平滑地流放到 1000 米外
            ...
            // 4. 🛡️ 极其关键的物理低通滤波器 (10Hz 阻尼)
            // 即使感知输出在一帧内突变，虚拟障碍物在小车视野中的移动速度也会被平滑限制，给 SQP 求解器留出充足的收敛时间！
            let 障碍物阻尼系数 = 0.06f64; 
            if 目标障碍物_x > 500.0 {
                // 如果是退出避障，允许快速流放
                滤波后的障碍物_x = 目标障碍物_x;
                ...
                if 滤波后的障碍物_x > 500.0 {
                    // 如果是从安全状态首次进入避障，瞬间初始化其位置，避免从小车屁股后面“飞”过来
                    滤波后的障碍物_x = 目标障碍物_x;
                    滤波后的障碍物_y = 目标障碍物_y;
                } else {
            ...
            // 将低通平滑后的硬约束坐标物理烧录进 NMPC 求解器
            if let Err(e) = 规控大脑.设置动态障碍物硬约束(滤波后的障碍物_x, 滤波后的障碍物_y, 椭圆长轴_a, 椭圆短轴_b) {
                eprintln!("⚠️ 动态障碍物硬约束注入失败: {}", e);
                continue;
            }
            ...
            match 规控大脑.求解最优控制量(当前线速度) {
                Ok((线速度_v, 角速度_w)) => {
                    ...
                    // 📊 2026 工业级数值探针
                    if 循环计数 % 100 == 0 {
                        let 禁区状态 = if 滤波后的障碍物_x < 100.0 { "🔴 禁区激活" } else { "🟢 畅通无阻" };
                        println!(
                            "[快大脑 100Hz 遥测] 步数: {:<6} | 状态: {} | 目标线速: {:.3} m/s | 避障偏置: {:.3} m | NMPC输出 -> v: {:.3} m/s, w: {:.3} rad/s",
                            循环计数, 禁区状态, 目标线速度, 期望_y, 线速度_v, 角速度_w
                        );
                    }
