...
async fn main() -> eyre::Result<()> {
    ...
    let 规控句柄 = tokio::spawn(async move {
        ...
        loop {
            ...
            // 🎯 战役三第六版：滤波系统响应升级
            // 由于感知端已经通过经典极速 FAST 实现了极高鲁棒性的物理特征遮罩，
            // 静态背景噪点已被彻底物理抹除。我们直接废除带来致命相位滞后的超慢阻尼（0.015），
            // 恢复高响应度、物理对齐的 120ms 级一阶低通滤波，彻底消灭控制时差！
            let 侧向滤波系数 = 0.125f64; 
            滤波后的期望_x += 0.125f64 * (有效期望_x - 滤波后的期望_x);
            ...
            if 循环计数 % 100 == 0 {
                ...
                println!(
                    "[快大脑 100Hz 遥测] 步数: {:<6} | 状态: {} | 实测线速: {:.3} m/s | 实际打舵: {:.3} rad/s",
                    循环计数, 禁区状态, 线速度_v, 角速度_w
                );
            }
