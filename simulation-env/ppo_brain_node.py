...
def main():
    print("=========================================================================")
    print("🧠  NEXUS - 25D Spiced PPO Neural Brain Deployment Node (SOTA 2026)")
    print("=========================================================================")
    ...
    state_force = [0.0, 0.0]         # fx, fy
    goal_pose = [0.52, 4.11]         # Fallback default goal
    
    # 5-frame sliding window history
    history_queue = deque(maxlen=5)
    
    print("[+] PPO Neural Brain successfully mounted. Entering 100Hz inference loop...")
    print("-" * 73)
    ...
    try:
        while True:
            ...
            if ev_type == "INPUT":
                ...
                # A. Update latest goal from slow brain mapper
                if ev_id == "human_prior":
                    prior_arr = event["value"].to_numpy()
                    if len(prior_arr) >= 2:
                        goal_pose = [float(prior_arr[0]), float(prior_arr[1])]
                        
                # B. Update latest obstacle forces
                elif ev_id == "obstacle_force":
                    force_arr = event["value"].to_numpy()
                    if len(force_arr) >= 2:
                        state_force = [float(force_arr[0]), float(force_arr[1])]
                        
                # C. Trigger 100Hz PPO inference upon receiving Odometry
                elif ev_id == "odometry":
                    odom_arr = event["value"].to_numpy()
                    if len(odom_arr) >= 3:
                        state_odom = [float(odom_arr[0]), float(odom_arr[1]), float(odom_arr[2])]
                        
                    # Calculate relative goal vector (PointGoal-Nav)
                    ...
                    # Assemble single 5D state representation with scale mapping
                    # [local_goal_x * 0.20, local_goal_y * 0.20, local_goal_dist * 0.20, f_x, f_y]
                    current_frame = [
                        local_goal_x * 0.20,
                        local_goal_y * 0.20,
                    ...
                    # Flatten sliding window to 25D input tensor
                    state_25 = np.array(list(history_queue), dtype=np.float32).flatten().reshape(1, 25)
                    
                    # Run ONNX inference
                    run_outs = session.run(None, {input_name: state_25})
                    action = run_outs[0][0] # shape [2] (v_scale, w_curvature)
                    
                    a_vel = float(action[0])
                    a_kappa = float(action[1])
                    
                    # Decode platform-agnostic commands via PAM
                    v_max = 0.80
                    kappa_max = 1.25
                    w_max = 1.00
                    ...
                    # Singularity Autorecovery for stationary pivot turns
                    if abs(v_des) < 0.05:
                        w_ref = np.clip(a_kappa, -1.0, 1.0) * w_max
                    else:
                        w_ref = kappa * v_des
                        
                    # Send commanded velocities to total bus
