...
class MahonyAHRS:
    ...
    def get_estimated_yaw(self):
        ...
        q = self.q
        return np.arctan2(2*(q[1]*q[2] + q[0]*q[3]), 1 - 2*(q[2]**2 + q[3]**2))
...
acados_solver = AcadosOcpSolver(None, json_file="acados_ocp.json", generate=False, build=False)
...
# 动态获取电机的执行通道控制索引
try:
    actuator_left_id = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_ACTUATOR, "motor_left")
    actuator_right_id = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_ACTUATOR, "motor_right")
except:
    actuator_left_id = 0
    actuator_right_id = 1
...
log_file.write("Time,Current_X,Current_Y,Ref_X,Ref_Y,L2_Error_m,Cmd_v,Cmd_w,Est_Yaw,True_Yaw,SolveTime_ms\n")
...
with mujoco.viewer.launch_passive(model, data) as viewer:
    ...
    while viewer.is_running() and step < len(t_points) - N:
        ...
        true_yaw = np.arctan2(2*(true_q[0]*true_q[3] + true_q[1]*true_q[2]), 1 - 2*(true_q[2]**2 + true_q[3]**2))
        ...
        raw_ax = 2.0 * (true_q[1]*true_q[3] - true_q[0]*true_q[2]) * 9.81 + accel_noise[0]
        raw_ay = 2.0 * (true_q[0]*true_q[1] + true_q[2]*true_q[3]) * 9.81 + accel_noise[1]
        raw_az = (true_q[0]**2 - true_q[1]**2 - true_q[2]**2 + true_q[3]**2) * 9.81 + accel_noise[2]
        ...
        for k in range(N):
            ...
            ocp_yref = np.array([ref_x[idx], ref_y[idx], theta_ref, 0.3, 0.0, 0.0]) # 设定目标速度为 0.3 m/s
        ...
        if step % 50 == 0:
            target_x = ref_x[step]
            target_y = ref_y[step]
            dist_error = np.sqrt((current_state[0] - target_x)**2 + (current_state[1] - target_y)**2)
            
            # 记录真实姿态与估计姿态
            log_file.write(f"{data.time:.2f},{current_state[0]:.4f},{current_state[1]:.4f},"
                           f"{target_x:.4f},{target_y:.4f},{dist_error:.4f},"
                           f"{v_cmd:.3f},{w_cmd:.3f},{estimated_yaw:.4f},{true_yaw:.4f},{elapsed*1000:.1f}\n")
            
            # 终端实时简化遥测，不刷屏（1行完成）
            print(f"Time: {data.time:.2f}s | Err: {dist_error:.3f}m | Solve: {elapsed*1000:.1f}ms")
