...
class SpicedDataset(Dataset):
    def __init__(self, dataset_dir):
        ...
        print(f"Loading and purifying {len(csv_files)} files...")
        ...
        for file_path in csv_files:
            ...
            for r in valid_rows:
                # 5D Pure Ego-centric Rescale
                l_goal_x = float(r['local_goal_x']) * 0.20
                l_goal_y = float(r['local_goal_y']) * 0.20
                l_goal_dist = float(r['local_goal_dist']) * 0.20
                
                f_x = float(r['frog_eye_fx'])
                f_y = float(r['frog_eye_fy'])
                
                obs = np.array([
                    l_goal_x, l_goal_y, l_goal_dist, f_x, f_y
                ], dtype=np.float32)
                
                cmd_v = float(r['cmd_v'])
                cmd_w = float(r['cmd_w'])
                act = np.array([cmd_v, cmd_w], dtype=np.float32)
        ...
        print(f"Purification completed. Total training samples: {len(self.inputs)}")
...
class GaussianPolicy(nn.Module):
    def __init__(self, input_dim=5, action_dim=2):
    ...
    def forward(self, x):
        ...
        mean_v = torch.sigmoid(raw_mean[:, 0:1]) * 1.0
        mean_w = torch.tanh(raw_mean[:, 1:2]) * 1.0
...
def train_bc():
    ...
    print("Beginning epoch-loop training (5D Causal-Cut & Steering Loss Balanced)...")
    for epoch in range(1, epochs + 1):
        ...
        for obs, act in train_loader:
            ...
            # Active steering frames are weighted 5.0x to overcome zero-steering bias
    ...
    print("Optimization completed. Training BC Anchor succeeded.")
    ...
    print(f"🏆 SUCCESS: Spiced Brain exported to {onnx_path}")
